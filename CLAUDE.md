...
- 現在のバージョン: v0.9（`js/views/settings.js` の `VERSION`）
...
- アーティスト登録: iTunes 候補から選択（写真付き表示）。写真は自動設定、候補から選び直し可。
  名前は**曲に付いている名義を優先**（iTunes のアーティスト名がローマ字「HARUKAMIRAI」でも曲は「ハルカミライ」のため）。
  曲一覧・ツアー候補のキャッシュは取得時の名前を覚えていて、名前を変えると取り直す。Wikipedia の曖昧さ回避ページは飛ばす。
...
- ライブ一覧: 先頭に次のライブを大きく表示（写真＋あと◯日）、チケットの左端にアーティスト色の帯、
  右上ボタンで写真グリッド表示に切り替え（端末ごとに記憶）。
- アーティスト色: components.js の rtistColor(id)（ID から決まる12色。名前を変えても同じ色）。
- 統計: 概要（アーティストのバブル＝回数に比例した大きさで全員表示。「その他」でまとめない。同数は最近行った順。ハイライト、支出内訳）、アーティスト（表彰台）、曲（上位3曲カード）、年、会場（座席の履歴）。
